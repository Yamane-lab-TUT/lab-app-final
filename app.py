...
# ==========================================
# 関数定義: page_graph_plotting (v37: MPPT単位表記修正版)
# ==========================================
def page_graph_plotting():
    st.header("📈 統合型グラフ解析ツール")
    st.markdown("""
    **v37 更新**: 
    - **MPPT単位表記修正**: 「電流密度に換算」がONの場合、MPPTの数値表示の単位にも `/cm²` (例: `mW/cm²`) が付くように修正しました。
    """)
    ...
    # 修正: 密度フラグを受け取り単位を調整
    def format_power(watts, is_density=False):
        w_abs = abs(watts)
        suffix = "/cm²" if is_density else ""
        
        if w_abs == 0: return f"0 W{suffix}"
        
        if w_abs >= 1: 
            return f"{w_abs:.3f} W{suffix}"
        elif w_abs >= 1e-3: 
            return f"{w_abs*1e3:.3f} mW{suffix}"
        elif w_abs >= 1e-6: 
            return f"{w_abs*1e6:.3f} µW{suffix}"
        elif w_abs >= 1e-9: 
            return f"{w_abs*1e9:.3f} nW{suffix}"
        else: 
            return f"{w_abs*1e12:.3f} pW{suffix}"
    ...
    with st.expander("💾 プロジェクトの保存・復元", expanded=False):
        ...
        with c_load:
            st.markdown("#### 📂 復元")
            uploaded_project = st.file_uploader("プロジェクトファイル (.json)", type=["json"], key="project_loader_v37")
            if uploaded_project:
                if st.button("設定を読み込む", key="btn_load_proj_v37"):
                    try:
        ...
        with c_save:
            ...
            save_name_proj = st.text_input("プロジェクト名 (拡張子不要)", value=default_proj_name, key="proj_save_name_v37")
            
            if st.button("プロジェクトファイルを作成", key="btn_save_proj_v37"):
                if not st.session_state['gp_data_list']:
                    ...
                    for key, val in st.session_state.items():
                        if key.startswith(("project_", "gp_", "btn_", "paste_", "fw_", "fh_", "dpi_", "ff_", "bfs_", "sleg", "lfont", "ax_preset", "legend_", "scale_sel", "vis_", "leg_nm_", "xc_", "yc_", "ut_", "ur_", "clr_", "mrk_", "lw_", "ms_", "lst_", "mppt_", "fit_", "seq_", "area_", "dens_", "mx_", "my_", "fill_", "xy_swap_", "proj_save_name", "img_save_name", "editor_")): continue
                    ...
                    st.download_button("⬇️ JSONをダウンロード", json_str, final_proj_fname, "application/json", key="dl_json_btn_v37")
    ...
    if st.session_state['gp_data_list']:
        ...
        if c_clear.button("🗑️ リストを全消去", key="btn_clear_all_v37"):
            st.session_state['gp_data_list'] = []
    ...
    with tab1:
        ...
        expand_cols = st.checkbox("列ごとに別の系列として追加する", value=False, key="expand_cols_v37")
        
        current_uploader_key = f"gp_uploader_v37_{st.session_state['uploader_key_id']}"
        uploaded_files = st.file_uploader("CSV/Excelファイルを選択", accept_multiple_files=True, key=current_uploader_key)
        
        if uploaded_files:
            if st.button(f"📂 {len(uploaded_files)}件のファイルをリストに追加", key="btn_add_files_v37"):
                count_added = 0
                ...
                if count_added > 0:
                    ...
                    st.warning("追加可能なファイルがありませんでした（重複またはエラー）")

    with tab2:
        st.markdown("**貼り付けオプション**")
        expand_paste = st.checkbox("列ごとに別の系列として追加する", value=False, key="expand_paste_v37")

        st.caption("Excelからコピペ (タブ区切り) して Ctrl+Enter")
        paste_text = st.text_area("データ貼り付けエリア", height=100, key="paste_area_v37")
        paste_name = st.text_input("データセット名", value=f"Data_{len(st.session_state['gp_data_list'])+1}", key="paste_name_v37")
        
        if st.button("貼り付け追加", key="btn_paste_add_v37"):
            if paste_text:
                try:
                    ...
                    if df_paste is not None and not df_paste.empty:
                        ...
                        if expand_paste and len(cols) >= 2:
                            x_c = cols[0]
                            for y_c in cols[1:]:
                                auto_color = get_next_color(len(st.session_state['gp_data_list']))
                                st.session_state['gp_data_list'].append({
                            ...
                            st.success(f"{len(cols)-1}件のデータを追加しました")
                            st.rerun()
                        else:
                            auto_color = get_next_color(len(st.session_state['gp_data_list']))
                            st.session_state['gp_data_list'].append({
    ...
    with col_settings:
        ...
        with st.expander("📊 キャンバス・フォント", expanded=False):
            c1, c2 = st.columns(2)
            fig_w_cm = c1.number_input("幅 (cm)", 2.0, 100.0, 15.0, step=0.5, key="fw_cm_v37")
            fig_h_cm = c2.number_input("高さ (cm)", 2.0, 100.0, 10.0, step=0.5, key="fh_cm_v37")
            
            fig_w_inch = fig_w_cm / 2.54
            fig_h_inch = fig_h_cm / 2.54
            
            dpi_val = st.number_input("解像度 (DPI)", 72, 600, 150, key="dpi_in_v37")
            
            font_options = ["🇯🇵 日本語標準 (Auto)", "Times New Roman", "Arial", "Helvetica", "Meiryo", "Yu Gothic"]
            font_family = st.selectbox("フォント", font_options, index=0, key="ff_sel_v37")
            base_fs = st.number_input("基本フォントサイズ", 6, 50, 12, key="bfs_in_v37")
        ...
        with st.expander("📐 軸 (Axes) と 単位変換", expanded=True):
            ...
            def axis_ui(key_prefix, label_def, use_top=False, use_right=False):
                is_secondary = use_top or use_right
                calc_mode = False
                forward_eq = "x"
                inverse_eq = "x"
                ...
                if is_secondary:
                    ...
                    mode_sel = st.radio("軸の種類", ["独立したデータ軸 (Data)", "第1軸からの計算値 (Function)"], 
                                      index=0, key=f"{key_prefix}_mode_v37", horizontal=True)
                    if "計算値" in mode_sel:
                        calc_mode = True
                        st.info("第1軸の値 `x` を使って計算します。目盛表示のため逆変換式も必要です。")
                        c_eq1, c_eq2 = st.columns(2)
                        forward_eq = c_eq1.text_input("変換式 (1軸→2軸)", "x**2", key=f"{key_prefix}_fw_eq")
                        inverse_eq = c_eq2.text_input("逆変換 (2軸→1軸)", "np.sqrt(x)", key=f"{key_prefix}_inv_eq")
                    else:
                        st.caption("他のデータをプロットするための独立した軸です。")
                    st.divider()

                if not calc_mode:
                    # --- プリセットボタン ---
                    # 1段目: 電圧/電流
                    col_btn1 = st.columns(4)
                    if col_btn1[0].button("V (V)", key=f"p_v_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Voltage (V)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 0
                    if col_btn1[1].button("I (A)", key=f"p_a_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Current (A)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 0
                    if col_btn1[2].button("I (mA)", key=f"p_ma_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Current (mA)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 1
                    if col_btn1[3].button("I (µA)", key=f"p_ua_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Current (µA)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 2
                    
                    # 2段目: 密度 (J)
                    col_btn2 = st.columns(3)
                    if col_btn2[0].button("J (A/cm²)", key=f"p_ja_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Current Density (A/cm²)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 0
                    if col_btn2[1].button("J (mA/cm²)", key=f"p_jma_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Current Density (mA/cm²)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 1
                    if col_btn2[2].button("P (mW/cm²)", key=f"p_pw_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v37"] = "Power Density (mW/cm²)"
                        st.session_state[f"{key_prefix}_scale_idx_v37"] = 1

                label = st.text_input("ラベル (日本語可)", label_def, key=f"{key_prefix}_lbl_v37")
                
                current_scale_val = 1.0
                if not calc_mode:
                    curr_idx = st.session_state.get(f"{key_prefix}_scale_idx_v37", 0)
                    scale_key = st.selectbox("表示倍率", list(SCALE_OPTIONS.keys()), index=curr_idx, key=f"{key_prefix}_scale_sel_v37")
                    st.session_state[f"{key_prefix}_scale_idx_v37"] = list(SCALE_OPTIONS.keys()).index(scale_key)
                    ...
                    if current_scale_val != prev_scale_val:
                        ...
                        k_min = f"{key_prefix}_min_v37"
                        k_max = f"{key_prefix}_max_v37"
                ...
                c3, c4 = st.columns(2)
                maj_int = c3.number_input("主目盛", 0.0, step=0.1, key=f"{key_prefix}_maj_v37")
                min_int = c4.number_input("補助目盛", 0.0, step=0.1, key=f"{key_prefix}_min_int_v37")
                
                c_sci, c_log = st.columns(2)
                sci_opts = ["Auto (端に表示)", "Scientific (各目盛 10^n)", "Plain (ベタ書き)"]
                sci_mode = c_sci.selectbox("指数表記形式", sci_opts, index=0, key=f"{key_prefix}_sci_v37")
                
                c5, c6 = st.columns(2)
                is_log = c5.checkbox("対数軸", False, key=f"{key_prefix}_log_v37")
                
                log_style = "10^n"
                if is_log:
                    log_style = c_log.selectbox("対数軸の表記", ["10^n", "数値 (0.1, 10...)"], index=0, key=f"{key_prefix}_log_sty_v37")
                
                is_inv = c6.checkbox("軸を反転", False, key=f"{key_prefix}_inv_v37")
                
                show_tick_lbl = st.checkbox("目盛数値を表示", True, key=f"{key_prefix}_stlbl_v37")

                return {
                    "label": label, "min": d_min, "max": d_max, "maj": maj_int, "min_int": min_int,
                    "log": is_log, "inv": is_inv, "scale": current_scale_val,
                    "sci_mode": sci_mode, "log_style": log_style,
                    "calc_mode": calc_mode, "fw_eq": forward_eq, "inv_eq": inverse_eq,
                    "show_tick_labels": show_tick_lbl
                }
            ...
            with tabs_ax[4]:
                tick_dir = st.selectbox("目盛の向き", ["in", "out", "inout"], index=0, key="tdir_v37")
                show_grid = st.checkbox("グリッド表示", False, key="sgrid_v37")
                zero_cross = st.checkbox("原点線描画", True, key="zcross_v37")

        # --- C. 凡例設定 ---
        with st.expander("📝 凡例 (Legend)", expanded=True):
            show_leg = st.checkbox("凡例を表示", True, key="sleg_v37")
            ...
            if show_leg:
                ...
                auto_leg_size = c_auto.checkbox("サイズ自動調整", True, key="auto_leg_size_v37")
                manual_fs = c_size.number_input("フォントサイズ", 5, 40, int(base_fs), disabled=auto_leg_size, key="lfont_v37")
                if auto_leg_size:
                ...
                c1, c2 = st.columns(2)
                l_loc = c1.selectbox("位置", ["best", "upper right", "upper left", "lower right", "lower left", "outside right"], index=0, key="lloc_v37")
                l_col = c2.number_input("列数", 1, 5, 1, key="lcol_v37")
                l_frame = st.checkbox("枠線を表示", False, key="lframe_v37")
        ...
        for i, d in enumerate(datasets):
            ...
            with st.expander(f"#{i+1}: {d.get('legend_name', d['name'])}", expanded=False):
                ...
                with bc3:
                    if st.button("©️ 複製", key=f"btn_dup_{did}"): duplicate_data(i); st.rerun()
                with bc4:
                    if st.button("❌ 削除", key=f"btn_del_{did}"): datasets.pop(i); st.rerun()

                with st.expander("🔢 生データを編集 (Excelライク)", expanded=False):
                    d['df'] = st.data_editor(d['df'], key=f"editor_{did}", num_rows="dynamic", use_container_width=True, height=200)
                ...
                if sc3.button("🔄 入替", key=f"xy_swap_{did}"):
                    ...
                    st.session_state[f"xc_{did}"] = yc
                    st.session_state[f"yc_{did}"] = xc
                    st.rerun()
    ...
    with col_preview:
        ...
        if "日本語" in font_family:
            ...
            plt.rcParams['font.sans-serif'] = [
                ...
                'Noto Sans CJK JP', 'Noto Sans JP', 'IPAGothic', 'TakaoGothic', 'sans-serif'
            ]
            plt.rcParams['mathtext.fontset'] = 'cm' 
        elif font_family in ["Times New Roman", "Times"]:
            plt.rcParams['font.family'] = 'serif'
            plt.rcParams['font.serif'] = [font_family] + plt.rcParams.get('font.serif', [])
            plt.rcParams['mathtext.fontset'] = 'cm'
        else:
            plt.rcParams['font.family'] = 'sans-serif'
            plt.rcParams['font.sans-serif'] = [font_family] + plt.rcParams.get('font.sans-serif', [])
        ...
        has_right_data = any(d.get('use_right') for d in visible_datasets)
        has_top_data = any(d.get('use_top') for d in visible_datasets)
        
        y2_is_calc = ax_settings['y2'].get('calc_mode', False)
        x2_is_calc = ax_settings['x2'].get('calc_mode', False)

        ax2, ax3 = None, None
        need_ax2_for_data = has_right_data and not y2_is_calc
        need_ax3_for_data = has_top_data and not x2_is_calc

        axes_map = {(False, False): ax1}

        if need_ax2_for_data:
            ax2 = ax1.twinx()
            axes_map[(False, True)] = ax2
        if need_ax3_for_data:
            ax3 = ax1.twiny()
            axes_map[(True, False)] = ax3
        if need_ax2_for_data and need_ax3_for_data:
            axes_map[(True, True)] = ax3 

        def safe_func_eval(expr, x_val):
            allowed = {"x": x_val, "np": np, "sqrt": np.sqrt, "log": np.log, "log10": np.log10, "exp": np.exp}
            try: return eval(expr, {"__builtins__": None}, allowed)
            except: return x_val 

        def apply_axis_conf(ax, axis_conf, axis_type='x', is_secondary_obj=False):
            if not ax: return
            if axis_type == 'x': ax.set_xlabel(axis_conf['label'])
            else: ax.set_ylabel(axis_conf['label'])
            
            if not is_secondary_obj:
                if axis_conf['min'] is not None and axis_conf['max'] is not None:
                    if axis_type == 'x': ax.set_xlim(axis_conf['min'], axis_conf['max'])
                    else: ax.set_ylim(axis_conf['min'], axis_conf['max'])
                elif axis_conf['min'] is not None:
                    if axis_type == 'x': ax.set_xlim(left=axis_conf['min'])
                    else: ax.set_ylim(bottom=axis_conf['min'])
                elif axis_conf['max'] is not None:
                    if axis_type == 'x': ax.set_xlim(right=axis_conf['max'])
                    else: ax.set_ylim(top=axis_conf['max'])

            is_log = axis_conf['log']
            target_axis = ax.xaxis if axis_type == 'x' else ax.yaxis
            
            if is_log:
                if not is_secondary_obj:
                    if axis_type == 'x': ax.set_xscale('log')
                    else: ax.set_yscale('log')
                log_sty = axis_conf.get('log_style', "10^n")
                if log_sty == "数値 (0.1, 10...)":
                    target_axis.set_major_formatter(ticker.ScalarFormatter())
                    target_axis.set_minor_formatter(ticker.ScalarFormatter())
            else:
                sci_mode = axis_conf.get('sci_mode', "Auto (端に表示)")
                if sci_mode == "Plain (ベタ書き)":
                    fmt = ticker.ScalarFormatter(useMathText=True)
                    fmt.set_powerlimits((-100, 100))
                    target_axis.set_major_formatter(fmt)
                elif sci_mode == "Scientific (各目盛 10^n)":
                    def sci_math_fmt(x, pos):
                        if x == 0: return "0"
                        exponent = int(np.floor(np.log10(abs(x))))
                        mantissa = x / (10**exponent)
                        return r"${:.1f} \times 10^{{{}}}$".format(mantissa, exponent)
                    target_axis.set_major_formatter(ticker.FuncFormatter(sci_math_fmt))
                else:
                    fmt = ticker.ScalarFormatter(useMathText=True)
                    fmt.set_powerlimits((-2, 4))
                    target_axis.set_major_formatter(fmt)

            if axis_conf.get('inv', False) and not is_secondary_obj:
                if axis_type == 'x': ax.invert_xaxis()
                else: ax.invert_yaxis()

            maj = axis_conf['maj']
            if maj > 0 and not is_log:
                target_axis.set_major_locator(ticker.MultipleLocator(maj))
            
            min_int = axis_conf.get('min_int', 0.0)
            if min_int > 0 and not is_log:
                target_axis.set_minor_locator(ticker.MultipleLocator(min_int))
            elif not is_log:
                if min_int == 0:
                    target_axis.set_minor_locator(ticker.NullLocator())
            
            if not axis_conf.get('show_tick_labels', True):
                if axis_type == 'x': ax.tick_params(axis='x', labelbottom=False, labeltop=False)
                else: ax.tick_params(axis='y', labelleft=False, labelright=False)
            
            ax.tick_params(direction=tick_dir, which='both')

        apply_axis_conf(ax1, ax_settings['x1'], 'x')
        apply_axis_conf(ax1, ax_settings['y1'], 'y')
        
        if ax2: apply_axis_conf(ax2, ax_settings['y2'], 'y')
        if ax3: apply_axis_conf(ax3, ax_settings['x2'], 'x')

        if y2_is_calc:
            def func_fw(x): return safe_func_eval(ax_settings['y2']['fw_eq'], x)
            def func_inv(x): return safe_func_eval(ax_settings['y2']['inv_eq'], x)
            try:
                sec_y = ax1.secondary_yaxis('right', functions=(func_fw, func_inv))
                apply_axis_conf(sec_y, ax_settings['y2'], 'y', is_secondary_obj=True)
            except Exception as e: st.warning(f"Y2軸エラー: {e}")

        if x2_is_calc:
            def func_fw(x): return safe_func_eval(ax_settings['x2']['fw_eq'], x)
            def func_inv(x): return safe_func_eval(ax_settings['x2']['inv_eq'], x)
            try:
                sec_x = ax1.secondary_xaxis('top', functions=(func_fw, func_inv))
                apply_axis_conf(sec_x, ax_settings['x2'], 'x', is_secondary_obj=True)
            except Exception as e: st.warning(f"X2軸エラー: {e}")
        ...
        for d in datasets:
            ...
            if use_r and y2_is_calc: use_r = False
            if use_t and x2_is_calc: use_t = False

            x_scale = ax_settings['x2']['scale'] if use_t and not x2_is_calc else ax_settings['x1']['scale']
            ...
            ls, mk = d.get('linestyle', '-'), d.get('marker', 'None')
            if ls == "None": ls = ""
            if mk == "None": mk = ""
            ...
            if fmode != "なし" and len(x_plot) > 1:
                try:
                    ...
                    xs, ys = x_plot.iloc[idx_sorted], y_plot.iloc[idx_sorted]
                    y_fit = None
                    eq_text = ""
                    if "線形" in fmode:
            ...
            if d.get('mppt'):
                if len(x_raw_plot) > 0:
                    p_raw = x_raw_plot * y_raw_plot
                    gen_mask = p_raw < 0
                    if gen_mask.any():
                        max_idx_loc = p_raw[gen_mask].idxmin()
                    else:
                        max_idx_loc = p_raw.abs().idxmax()
                    ...
                    # 単位suffixを渡す
                    is_density = d.get('use_density', False)
                    pow_str = format_power(best_p_raw, is_density=is_density)

                    target_ax.plot(best_x_plot, best_y_plot, marker='*', color='gold', markersize=14, markeredgecolor='black', zorder=10)
                    off_x, off_y = d.get('mppt_x', 10), d.get('mppt_y', -30)
        ...
        if show_leg and legend_handles:
            ...
            ax1.legend(legend_handles, legend_labels, loc=loc_param, bbox_to_anchor=bbox, ncol=l_col, fontsize=l_fontsize, frameon=l_frame, edgecolor='black')
        ...
        buf = BytesIO()
        try: fig.savefig(buf, format="png", dpi=300, bbox_inches='tight')
        except: 
            buf = BytesIO()
            fig.savefig(buf, format="png", dpi=300)
        
        default_img_name = f"plot_{datetime.now().strftime('%Y%m%d_%H%M')}"
        save_name_img = st.text_input("画像保存名 (拡張子不要)", value=default_img_name, key="img_save_name_v37")
        ...
        st.download_button("画像を保存 (PNG)", buf.getvalue(), final_img_fname, "image/png", key="dl_png_v37")

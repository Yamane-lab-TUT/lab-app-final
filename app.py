# -*- coding: utf-8 -*-
"""
bennriyasann3_final_integrated_complete.py
Yamane Lab Convenience Tool - 完全機能復元 + 安全な認証統合版
"""
...
# ---------------------------
# --- Google Service Stubs (ダミークライアント) ---
# ---------------------------
class DummyGSClient:
...
def initialize_google_services():
    ...
    if storage is None:
        ...
        # storage_clientはダミーを返す
        return DummyGSClient(), DummyStorageClient()

    # secretsのチェック (gcs_credentials または gcp_service_account)
    creds_info = None
    ...
    if creds_info:
        try:
            # Gspread認証
            gc_real = gspread.service_account_from_dict(creds_info)
            # GCS認証
            storage_real = storage.Client.from_service_account_info(creds_info)
        ...
        st.sidebar.info("Streamlit secrets に `gcs_credentials` または `gcp_service_account` を設定してください（オフラインモード）。")
        return DummyGSClient(), DummyStorageClient()
...
gcal_service = None
try:
    # 認証情報が取得できている場合のみカレンダー初期化を試みる
    if not isinstance(gc, DummyGSClient):
        # 再度secretsから情報を取得する必要があるが、簡略化のためsecretsを直接参照
        if "gcs_credentials" in st.secrets:
            raw = st.secrets["gcs_credentials"]
            cleaned = raw.strip().replace('\t', '').replace('\r', '').replace('\n', '')
            creds_info = json.loads(cleaned)
        ...
        if creds_info:
            gcal_creds = service_account.Credentials.from_service_account_info(creds_info, scopes=SCOPES)
...
def get_data_from_gspread(sheet_name):
    ...
    try:
        ...
        # シート読み込みエラーは致命的でない限り表示しないか、警告にとどめる
...
def save_row_to_sheet(sheet_name, row_data):
    ...
    if isinstance(gc, DummyGSClient):
        st.error("認証されていないため保存できません（オフラインモード）。")
        return

...
# ==========================================
# 関数定義: page_graph_plotting (v30: 第2軸の計算値モード追加版)
# ==========================================
def page_graph_plotting():
    st.header("📈 統合型グラフ解析ツール")
    st.markdown("""
    **v30 更新**: 
    - **第2軸の換算モード**: X2/Y2軸を、第1軸の計算値（例: $x^2$, $1/x$など）として表示できるようになりました。
    """)
    ...
    with st.expander("💾 プロジェクトの保存・復元", expanded=False):
        ...
        with c_load:
            st.markdown("#### 📂 復元")
            uploaded_project = st.file_uploader("プロジェクトファイル (.json)", type=["json"], key="project_loader_v30")
            if uploaded_project:
                if st.button("設定を読み込む", key="btn_load_proj_v30"):
                    try:
        ...
        with c_save:
            ...
            save_name_proj = st.text_input("プロジェクト名 (拡張子不要)", value=default_proj_name, key="proj_save_name_v30")
            
            if st.button("プロジェクトファイルを作成", key="btn_save_proj_v30"):
                if not st.session_state['gp_data_list']:
                    ...
                    st.download_button("⬇️ JSONをダウンロード", json_str, final_proj_fname, "application/json", key="dl_json_btn_v30")
    ...
    if st.session_state['gp_data_list']:
        ...
        if st.button("🗑️ 全データをクリア", key="btn_clear_all_v30"):
            st.session_state['gp_data_list'] = []
    ...
    with tab1:
        ...
        expand_cols = st.checkbox("列ごとに別の系列として追加する", value=False, key="expand_cols_v30")
        
        current_uploader_key = f"gp_uploader_v30_{st.session_state['uploader_key_id']}"
    ...
    with tab2:
        ...
        expand_paste = st.checkbox("列ごとに別の系列として追加する", value=False, key="expand_paste_v30")

        st.caption("Excelからコピペ (タブ区切り) して Ctrl+Enter")
        paste_text = st.text_area("データ貼り付けエリア", height=100, key="paste_area_v30")
        paste_name = st.text_input("データセット名", value=f"Data_{len(st.session_state['gp_data_list'])+1}", key="paste_name_v30")
        
        if st.button("貼り付け追加", key="btn_paste_add_v30"):
            if paste_text:
                try:
    ...
    with col_settings:
        ...
        with st.expander("📊 キャンバス・フォント", expanded=False):
            c1, c2 = st.columns(2)
            fig_w_cm = c1.number_input("幅 (cm)", 2.0, 100.0, 15.0, step=0.5, key="fw_cm_v30")
            fig_h_cm = c2.number_input("高さ (cm)", 2.0, 100.0, 10.0, step=0.5, key="fh_cm_v30")
            
            fig_w_inch = fig_w_cm / 2.54
            fig_h_inch = fig_h_cm / 2.54
            
            dpi_val = st.number_input("解像度 (DPI)", 72, 600, 150, key="dpi_in_v30")
            
            font_options = ["🇯🇵 日本語標準 (Auto)", "Times New Roman", "Arial", "Helvetica", "Meiryo", "Yu Gothic"]
            font_family = st.selectbox("フォント", font_options, index=0, key="ff_sel_v30")
            base_fs = st.number_input("基本フォントサイズ", 6, 50, 12, key="bfs_in_v30")
        ...
        with st.expander("📐 軸 (Axes) と 単位変換", expanded=True):
            ...
            def axis_ui(key_prefix, label_def, use_top=False, use_right=False):
                ...
                if is_secondary:
                    ...
                    mode_sel = st.radio("軸の種類", ["独立したデータ軸 (Data)", "第1軸からの計算値 (Function)"], 
                                      index=0, key=f"{key_prefix}_mode_v30", horizontal=True)
                    if "計算値" in mode_sel:
                        calc_mode = True
                ...
                if not calc_mode:
                    ...
                    if col_btn[0].button("Voltage(V)", key=f"p_v_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v30"] = "Voltage (V)"
                        st.session_state[f"{key_prefix}_scale_idx_v30"] = 0
                    if col_btn[1].button("Current(mA)", key=f"p_ma_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v30"] = "Current (mA)"
                        st.session_state[f"{key_prefix}_scale_idx_v30"] = 1
                    if col_btn[2].button("Current(µA)", key=f"p_ua_{key_prefix}"):
                        st.session_state[f"{key_prefix}_lbl_v30"] = "Current (µA)"
                        st.session_state[f"{key_prefix}_scale_idx_v30"] = 2

                label = st.text_input("ラベル (日本語可)", label_def, key=f"{key_prefix}_lbl_v30")
                
                # --- スケール設定 (通常モードのみ) ---
                current_scale_val = 1.0
                if not calc_mode:
                    curr_idx = st.session_state.get(f"{key_prefix}_scale_idx_v30", 0)
                    scale_key = st.selectbox("表示倍率", list(SCALE_OPTIONS.keys()), index=curr_idx, key=f"{key_prefix}_scale_sel_v30")
                    st.session_state[f"{key_prefix}_scale_idx_v30"] = list(SCALE_OPTIONS.keys()).index(scale_key)
                    ...
                    if current_scale_val != prev_scale_val:
                        ...
                        k_min = f"{key_prefix}_min_v30"
                        k_max = f"{key_prefix}_max_v30"
                ...
                # --- 共通設定 ---
                c3, c4 = st.columns(2)
                maj_int = c3.number_input("主目盛", 0.0, step=0.1, key=f"{key_prefix}_maj_v30")
                min_int = c4.number_input("補助目盛", 0.0, step=0.1, key=f"{key_prefix}_min_int_v30")
                
                c_sci, c_log = st.columns(2)
                sci_mode = c_sci.selectbox("指数表記(x10^n)", ["自動", "OFF (ベタ書き)", "ON (強制)"], index=0, key=f"{key_prefix}_sci_v30")
                
                c5, c6 = st.columns(2)
                is_log = c5.checkbox("対数軸", False, key=f"{key_prefix}_log_v30")
                
                log_style = "10^n"
                if is_log:
                    log_style = c_log.selectbox("対数軸の表記", ["10^n", "数値 (0.1, 10...)"], index=0, key=f"{key_prefix}_log_sty_v30")
                
                is_inv = c6.checkbox("軸を反転", False, key=f"{key_prefix}_inv_v30")
            ...
            with tabs_ax[4]:
                tick_dir = st.selectbox("目盛の向き", ["in", "out", "inout"], index=0, key="tdir_v30")
                show_grid = st.checkbox("グリッド表示", False, key="sgrid_v30")
                zero_cross = st.checkbox("原点線描画", True, key="zcross_v30")

        # --- C. 凡例設定 ---
        with st.expander("📝 凡例 (Legend)", expanded=True):
            show_leg = st.checkbox("凡例を表示", True, key="sleg_v30")
            ...
            if show_leg:
                ...
                auto_leg_size = c_auto.checkbox("サイズ自動調整", True, key="auto_leg_size_v30")
                manual_fs = c_size.number_input("フォントサイズ", 5, 40, int(base_fs), disabled=auto_leg_size, key="lfont_v30")
                if auto_leg_size:
                ...
                c1, c2 = st.columns(2)
                l_loc = c1.selectbox("位置", ["best", "upper right", "upper left", "lower right", "lower left", "outside right"], index=0, key="lloc_v30")
                l_col = c2.number_input("列数", 1, 5, 1, key="lcol_v30")
                l_frame = st.checkbox("枠線を表示", False, key="lframe_v30")
        ...
        for i, d in enumerate(datasets):
            ...
            with st.expander(f"#{i+1}: {d.get('legend_name', d['name'])}", expanded=False):
                ...
                with st.expander("🔢 生データを編集 (Excelライク)", expanded=False):
                    st.caption("値をダブルクリックで編集。行選択してDelで削除。一番下の行に追加可能。")
                    d['df'] = st.data_editor(
                        d['df'],
                        key=f"editor_{did}",
                        num_rows="dynamic",
    ...
    with col_preview:
        ...
        # Y2が「計算モード」なら、データ用のtwinxは作らず、secondary_yaxisを作る
        y2_is_calc = ax_settings['y2'].get('calc_mode', False)
        x2_is_calc = ax_settings['x2'].get('calc_mode', False)

        ax2, ax3 = None, None
        
        # データプロット用の軸が必要かどうかの判断
        # (計算モードの場合は、データプロット用ではなく装飾用になる)
        ...
        if need_ax2_for_data and need_ax3_for_data:
            axes_map[(True, True)] = ax3 # 簡易的に上軸優先

        # --- 安全なeval関数 ---
        def safe_func_eval(expr, x_val):
            allowed = {
                "x": x_val,
                "np": np, "sqrt": np.sqrt, "log": np.log, "log10": np.log10, 
                "exp": np.exp, "sin": np.sin, "cos": np.cos, "tan": np.tan, "pi": np.pi, "power": np.power
            }
            try:
                return eval(expr, {"__builtins__": None}, allowed)
            except:
                return x_val # エラー時はそのまま返す
        ...
        def apply_axis_conf(ax, axis_conf, axis_type='x', is_secondary_obj=False):
            ...
            if is_log:
                # 計算軸の場合、secondary_yaxisがスケールを管理するためここでset_scaleしないこともあるが、
                # Matplotlibの仕様上、親がlogなら子もその変換を受ける。
                if not is_secondary_obj:
                ...
                if sci_mode == "OFF (ベタ書き)": fmt.set_powerlimits((-100, 100))
                elif sci_mode == "ON (強制)": fmt.set_powerlimits((0, 0))
                else: fmt.set_powerlimits((-2, 4))
        ...
        # --- 計算軸 (Secondary Axis) の作成と適用 ---
        if y2_is_calc:
            # 関数定義
            fw_str = ax_settings['y2']['fw_eq']
            inv_str = ax_settings['y2']['inv_eq']
            
            def func_fw(x): return safe_func_eval(fw_str, x)
            def func_inv(x): return safe_func_eval(inv_str, x)
            ...
            try:
                ...
                st.warning(f"Y2軸の数式変換に失敗しました: {e}")

        if x2_is_calc:
            fw_str = ax_settings['x2']['fw_eq']
            inv_str = ax_settings['x2']['inv_eq']
            ...
            try:
                ...
                st.warning(f"X2軸の数式変換に失敗しました: {e}")
        ...
        for d in datasets:
            ...
            # 計算モードの軸を使おうとしている場合、強制的に第1軸に戻すか警告する
            # ここでは安全のため、計算モードの軸にはプロットできないようにする（ax1にフォールバック）
        ...
        save_name_img = st.text_input("画像保存名 (拡張子不要)", value=default_img_name, key="img_save_name_v30")
        ...
        st.download_button("画像を保存 (PNG)", buf.getvalue(), final_img_fname, "image/png", key="dl_png_v30")
# ---------------------------
# --- Components ---
# ---------------------------

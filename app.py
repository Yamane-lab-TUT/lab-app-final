...
# --- IVデータ解析用ユーティリティ ---
# @st.cache_data を追加し、一度解析したファイルはキャッシュする
...
def load_iv_data(uploaded_file_bytes, uploaded_file_name):
    ...
    try:
        ...
        if not cleaned_data_lines:
            st.warning(f"警告：'{uploaded_file_name}'に有効なデータが含まれていません。")
            return None

        # クリーンアップされたデータを行として結合
        data_string_io = io.StringIO("\n".join(cleaned_data_lines))
        
        # --- 既存のロバストな読み込みロジックを維持しつつ、手動スキップに対応 ---
        # skiprows=0 (ヘッダーは既に手動でスキップ済み), header=None
        common_read_params = {'skiprows': 0, 'header': None} 
        ...
        try:
            ...
            try:
                ...
                # 3. 失敗した場合、ロバストなPythonエンジンで \s+ または , 区切りで読み込む
                data_string_io.seek(0)
                df = pd.read_csv(data_string_io, sep=r'\s+|,', engine='python', **common_read_params)

        # 2列目以降を削除し、列名を再設定 (列は0, 1)
        if df is None or len(df.columns) < 2:
            st.warning(f"警告：'{uploaded_file_name}'の読み込みに失敗しました。ファイル形式を確認してください。")
            return None
        ...
        df[uploaded_file_name] = pd.to_numeric(df[uploaded_file_name], errors='coerce')
        df.dropna(inplace=True)
        ...
        st.error(f"エラー：'{uploaded_file_name}'の読み込みに失敗しました。ファイル形式を確認してください。({e})")
        return None

# -------------------------------------------------------------
# 【新規追加】データ結合処理をキャッシュするための関数
# -------------------------------------------------------------

@st.cache_data(show_spinner="データを結合中...")
def combine_iv_dataframes(dataframes, filenames):
    """
    複数のIV DataFrameをVoltage_Vをキーに外部結合し、Excelに書き込むための処理を高速化する。
    """
    if not dataframes:
        return None
    
    # 最初のDataFrameをベースにする
    combined_df = dataframes[0].rename(columns={dataframes[0].columns[1]: filenames[0]})
    
    # 残りのDataFrameをVoltage_Vをキーに外部結合（マージ）
    for i in range(1, len(dataframes)):
        # カラム名を元のファイル名にリネーム
        df_to_merge = dataframes[i].rename(columns={dataframes[i].columns[1]: filenames[i]})
        
        # Voltage_Vをキーにして外部結合 (全てのVoltage値を残す)
        # joinではなくpd.mergeを使用することで、処理速度とメモリ効率を向上
    ...
    # Current_A の列をすべて小数点以下4桁に丸める（表示の整理のため）
...
def page_iv_analysis():
    ...
    if uploaded_files:
        ...
        if valid_dataframes:
            ...
            # Matplotlibを使用してプロット
            fig, ax = plt.subplots(figsize=(10, 6)) # グラフサイズを大きくする
            ...
            # Excelダウンロード (ExcelWriterを使用し、メモリ効率を考慮)
            output = BytesIO()

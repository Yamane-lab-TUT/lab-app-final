...
# ==========================================
# 関数定義: page_graph_plotting (KeyError修正版 v12)
# ==========================================
def page_graph_plotting():
    st.header("📈 統合型グラフ解析ツール")
    st.markdown("初期化エラー修正済み：アップロード直後の動作を安定させました。")
    ...
    with st.expander("💾 プロジェクトの保存・復元", expanded=False):
        ...
        with c_load:
            st.markdown("#### 📂 復元")
            uploaded_project = st.file_uploader("プロジェクトファイル (.json)", type=["json"], key="project_loader_v12")
            if uploaded_project:
                if st.button("設定を読み込む", key="btn_load_proj_v12"):
                    try:
        ...
        with c_save:
            st.markdown("#### 💾 保存")
            if st.button("プロジェクトファイルを作成", key="btn_save_proj_v12"):
                if not st.session_state['gp_data_list']:
                    ...
                    for key, val in st.session_state.items():
                        if key.startswith(("project_", "gp_", "btn_", "paste_", "fw_", "fh_", "dpi_", "ff_", "bfs_", "sleg", "lfont", "ax_preset", "legend_", "scale_sel")): continue
                    ...
                    st.download_button("⬇️ JSONをダウンロード", json_str, file_name, "application/json", key="dl_json_btn_v12")
    ...
    if st.session_state['gp_data_list']:
        ...
        if st.button("🗑️ 全データをクリア", key="btn_clear_all_v12"):
            st.session_state['gp_data_list'] = []
            st.rerun()
    ...
    with tab1:
        files = st.file_uploader("CSV/Excelファイル", accept_multiple_files=True, key="gp_uploader_v12")
        if files:
    ...
    with tab2:
        ...
        paste_text = st.text_area("データ貼り付け", height=100, key="paste_area_v12")
        paste_name = st.text_input("データ名", value=f"Data_{len(st.session_state['gp_data_list'])+1}", key="paste_name_v12")
        if st.button("貼り付け追加", key="btn_paste_add_v12"):
            if paste_text:
                try:
    ...
    with col_settings:
        ...
        with st.expander("📊 キャンバス・フォント", expanded=False):
            c1, c2 = st.columns(2)
            fig_w = c1.number_input("幅 (inch)", 1.0, 50.0, 6.0, step=0.5, key="fw_in_v12")
            fig_h = c2.number_input("高さ (inch)", 1.0, 50.0, 4.0, step=0.5, key="fh_in_v12")
            dpi_val = st.number_input("解像度 (DPI)", 72, 600, 150, key="dpi_in_v12")
            
            font_family = st.selectbox("フォント", ["Times New Roman", "Arial", "Helvetica", "Meiryo", "Yu Gothic"], index=0, key="ff_sel_v12")
            base_fs = st.number_input("基本フォントサイズ", 6, 50, 12, key="bfs_in_v12")
        ...
        with st.expander("📐 軸 (Axes) と 単位変換", expanded=True):
            ...
            def axis_ui(key_prefix, label_def, use_top=False, use_right=False):
                ...
                if col_btn[0].button("Voltage(V)", key=f"p_v_{key_prefix}"):
                    st.session_state[f"{key_prefix}_lbl_v12"] = "Voltage (V)"
                    st.session_state[f"{key_prefix}_scale_idx_v12"] = 0
                if col_btn[1].button("Current(mA)", key=f"p_ma_{key_prefix}"):
                    st.session_state[f"{key_prefix}_lbl_v12"] = "Current (mA)"
                    st.session_state[f"{key_prefix}_scale_idx_v12"] = 1
                if col_btn[2].button("Current(µA)", key=f"p_ua_{key_prefix}"):
                    st.session_state[f"{key_prefix}_lbl_v12"] = "Current (µA)"
                    st.session_state[f"{key_prefix}_scale_idx_v12"] = 2

                label = st.text_input("ラベル", label_def, key=f"{key_prefix}_lbl_v12")
                
                curr_idx = st.session_state.get(f"{key_prefix}_scale_idx_v12", 0)
                scale_key = st.selectbox("表示倍率", list(SCALE_OPTIONS.keys()), index=curr_idx, key=f"{key_prefix}_scale_sel_v12")
                st.session_state[f"{key_prefix}_scale_idx_v12"] = list(SCALE_OPTIONS.keys()).index(scale_key)
                ...
                if current_scale_val != prev_scale_val:
                    ...
                    k_min = f"{key_prefix}_min_v12"
                    k_max = f"{key_prefix}_max_v12"
                ...
                for d in datasets:
                    ...
                    if key_prefix.startswith('x') and is_this_axis_x:
                        data_vals.append(d['df'][d['x_col']] * current_scale_val)
                    elif key_prefix.startswith('y') and is_this_axis_y:
                        data_vals.append(d['df'][d['y_col']] * current_scale_val)
                ...
                c3, c4 = st.columns(2)
                maj_int = c3.number_input("主目盛", 0.0, step=0.1, key=f"{key_prefix}_maj_v12")
                min_int = c4.number_input("補助目盛", 0.0, step=0.1, key=f"{key_prefix}_min_int_v12")
                is_log = st.checkbox("対数軸", False, key=f"{key_prefix}_log_v12")
            ...
            with tabs_ax[4]:
                tick_dir = st.selectbox("目盛の向き", ["in", "out", "inout"], index=0, key="tdir_v12")
                show_grid = st.checkbox("グリッド表示", True, key="sgrid_v12")
                zero_cross = st.checkbox("原点線描画", True, key="zcross_v12")

        # --- C. 凡例設定 ---
        with st.expander("📝 凡例 (Legend)", expanded=True):
            show_leg = st.checkbox("凡例を表示", True, key="sleg_v12")
            ...
            for i, d in enumerate(datasets):
                ...
                with c_vis:
                    d['visible'] = st.checkbox("vis", value=d.get('visible', True), key=f"vis_main_{i}", label_visibility="collapsed")
                with c_name:
                    st.text(f"{d.get('legend_name', d['name'])}")
                with c_up:
                    if st.button("⬆", key=f"leg_u_{i}"): move_data(i, "up"); st.rerun()
                with c_down:
                    if st.button("⬇", key=f"leg_d_{i}"): move_data(i, "down"); st.rerun()

            if show_leg:
                st.markdown("---")
                st.markdown("**スタイル設定**")
                c_auto, c_size = st.columns(2)
                auto_leg_size = c_auto.checkbox("サイズ自動調整", True, key="auto_leg_size_v12")
                manual_fs = c_size.number_input("フォントサイズ", 5, 40, int(base_fs), disabled=auto_leg_size, key="lfont_v12")
                ...
                c1, c2 = st.columns(2)
                l_loc = c1.selectbox("位置", ["best", "upper right", "upper left", "lower right", "lower left", "outside right"], index=0, key="lloc_v12")
                l_col = c2.number_input("列数", 1, 5, 1, key="lcol_v12")
                l_frame = st.checkbox("枠線を表示", False, key="lframe_v12")
        ...
        for i, d in enumerate(datasets):
            with st.expander(f"#{i+1}: {d.get('legend_name', d['name'])}", expanded=False):
                ...
                d['legend_name'] = st.text_input("凡例表示名", value=d.get('legend_name', d['name']), key=f"leg_nm_{i}_v12")

                bc1, bc2, bc3 = st.columns([1, 1, 2])
                with bc1:
                    if st.button("⬆", key=f"btn_u_{i}_v12"): move_data(i, "up"); st.rerun()
                with bc2:
                    if st.button("⬇", key=f"btn_d_{i}_v12"): move_data(i, "down"); st.rerun()
                with bc3:
                    if st.button("❌ 削除", key=f"btn_del_{i}_v12"): datasets.pop(i); st.rerun()

                cols = d['df'].columns.tolist()
                sc1, sc2 = st.columns(2)
                xc = sc1.selectbox(f"X列", cols, index=0 if cols else None, key=f"xc_{i}_v12")
                # 2列目があればそれをY、なければ1列目をY
                def_y = 1 if len(cols) > 1 else 0
                yc = sc2.selectbox(f"Y列", cols, index=def_y if cols else None, key=f"yc_{i}_v12")
                
                ac1, ac2 = st.columns(2)
                d['use_top'] = ac1.checkbox("上X軸", d.get('use_top', False), key=f"ut_{i}_v12")
                d['use_right'] = ac2.checkbox("右Y軸", d.get('use_right', False), key=f"ur_{i}_v12")

                tc1, tc2 = st.columns(2)
                d['color'] = tc1.color_picker("色", d.get('color', '#0000FF'), key=f"clr_{i}_v12")
                d['marker'] = tc2.selectbox("マーカー", ["None", "o", "s", "^", "x"], index=0 if d.get('marker')=="None" else 1, key=f"mrk_{i}_v12")
                lw1, lw2 = st.columns(2)
                d['line_width'] = lw1.number_input("線幅", 0.0, 10.0, float(d.get('line_width', 1.5)), key=f"lw_{i}_v12")
                d['marker_size'] = lw2.number_input("点サイズ", 0.0, 20.0, float(d.get('marker_size', 6.0)), key=f"ms_{i}_v12")
                d['linestyle'] = st.selectbox("線種", ["-", "--", "-.", ":", "None"], index=0, key=f"lst_{i}_v12")

                st.markdown("---")
                d['mppt'] = st.checkbox("MPPT解析", d.get('mppt', False), key=f"mppt_{i}_v12")
                d['fit_mode'] = st.selectbox("近似曲線", ["なし", "線形", "多項式(2次)", "移動平均"], index=0, key=f"fit_{i}_v12")
                if d['fit_mode'] != "なし":
                    d['show_eq'] = st.checkbox("数式を表示", d.get('show_eq', False), key=f"seq_{i}_v12")
    ...
    with col_preview:
        ...
        st.download_button("画像を保存 (PNG)", buf.getvalue(), "plot.png", "image/png", key="dl_png_v12")
